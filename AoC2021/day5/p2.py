...
def plotLines(lines, type):
	...
	if type == 3:
		for i in lines.reshape((-1,4)):
			xinc = np.sign(i[2]-i[0])
			yinc = np.sign(i[3]-i[1])
			xinter = i[0]
			yinter = i[1]
			while True:
				plotted = np.append(plotted, [xinter, yinter])
				if xinter == i[2]:
					break
				xinter = xinter + xinc*1
				yinter = yinter + yinc*1
...
plot3 = plotLines(dep,3)
plot = np.vstack((plot1, plot2, plot3))
...
for i in space.flatten():
	if i > 1:
		sum=sum+1
print(f'Total points where more than 1 lines cross: {sum}')
